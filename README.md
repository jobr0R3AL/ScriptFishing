...
ScriptFishing is a fun, interactive terminal-based fishing simulator written in Python. Simulate the thrill of fishing with a simple script that randomly determines whether you catch a fish or lose your catch!
...
3. **Random Fish Selection** - If you catch something, the script randomly selects from 10 different fish species:
   - Trout!
   - Bass!
   - Rainbow Trout!
   - Salmon!
...
4. **Fish Details** - Each successful catch includes:
   - **Weight**: Randomly generated between 1-18 kg
   - **Size**: Randomly generated between 2-36 inches
...
| `fish_types` | List of possible fish species |
| `weight` | Random weight in kg (1-18 range) |
| `size` | Random size in inches (2-36 range) |

The script uses Python's `random` module for all randomization and `time.sleep()` to create suspense before revealing the catch result. Each catch is logged to `fishing_log.txt`.
